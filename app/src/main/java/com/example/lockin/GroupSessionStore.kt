...
data class MemberStatus(val uid: String, val displayName: String, val state: ComplianceState)

fun pushLiveStatus(groupId: String, uid: String, displayName: String, status: ComplianceStatus) {
    val doc = mapOf(
    ...
    return Firebase.firestore.collection("groups").document(groupId).collection("liveStatus")
        .addSnapshotListener { snapshot, error ->
            ...
            val statuses = snapshot?.documents.orEmpty().mapNotNull { doc ->
                ...
                MemberStatus(doc.id, name, state)
            }
            onChange(statuses)
        }
}
