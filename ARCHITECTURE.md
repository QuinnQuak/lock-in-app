...
- **The Home header reflects the alarm, not just compliance (`alarmSounding`).** Because the alarm is sticky, a group breaker who returns to Lock-In is COMPLIANT again while the alarm still blares — the header used to read a misleading green "LOCK-IN ACTIVE" there. The service now publishes its settled `alarmActive` flag each tick as `LockInMonitor.alarmSounding`, and the header renders in precedence **BREAK ("BREAK DETECTED") → alarm ("ALARM SOUNDING", coral) → compliant ("LOCK-IN ACTIVE", green)**. Chose an explicit published flag over deriving `breakId > 0 && !isBreak` in the UI (which is *currently* equivalent, but only because of the loop's reset ordering) — same rationale as surfacing `breakId` for the mute flow: reflect ground truth, don't re-derive an invariant the UI shouldn't depend on.
