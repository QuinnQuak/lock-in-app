...
- **Extract REST-response fields with a real JSON parser, not `grep`/`cut`** — a `TOKEN=$(curl -s ... | grep -o '"idToken":"[^"]*"' | cut -d'"' -f4)` pipeline came back empty (`token len: 0`) even though the sign-in endpoint was returning a valid 929-char `idToken` — looked like an API failure but wasn't. Write the response to a file, then pull fields out with `python -c "import json; ...json.load(...)['idToken']"`. Robust against both fragile shell parsing and transient pipeline blips; the exact culprit was never isolated because saving-to-file and switching to Python fixed it together.
