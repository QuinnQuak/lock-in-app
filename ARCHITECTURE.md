...
- Dev/test accounts: `testuser@lockin.test` / `testuser2@lockin.test` (throwaway, trivial passwords, emulator-only).
...
| File | Responsibility |
|---|---|
| `MainActivity.kt` | Screens (Auth gate, Permission prompt, Home, Allowlist, Friends, Groups) + navigation shell; owns `AuthStateListener`, allowlist snapshot-listener, and group break-watcher lifecycles |
