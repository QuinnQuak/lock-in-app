...
- **Airplane mode / no connectivity:** Many blocked-category apps (games, cached social feeds, offline media, notes, texts) still work fully offline. Airplane mode doesn't stop the underlying behavior. *✅ Verified + refined by Stage 7 step 4:* it does **not** defeat the *local* detection + solo alarm (`UsageStatsManager` is a local query — the alarm was confirmed sounding while offline). It only *delays* the *group reporting* layer (liveStatus/mute/alerts, which need Firestore): a BREAK `liveStatus` write **queues offline and flushes on reconnect** (Firestore offline persistence), so the break reaches the group once connectivity returns as long as the process survives (if it's killed too, step 2 voids it locally). The group's ability to *see* a member go dark *during* the outage is deferred to Stage 8 (decision 3).
...
- **Build order (decided 2026-07-15, all built — see `docs/archive/STAGES_0-6.md` for the build log):** 1) palette + typography swap; 2) theme picker (Peach/Berry/Sunset + persisted selection); 3) mascot "blob buddy" (Canvas-drawn, app-state reactivity only, no economy); 4) Sparkles currency (1/min-locked-in accrual, display only); 5) trophy case (7 achievement tiers → auto-granted accessories, ownership derived on the fly); 6) Shop (spend Sparkles via a Firestore transaction, reusing the trophy case's equip slot + inventory). **Rationale:** visual foundation before the character before the economy; within the economy, the free/automatic unlock (trophy case) before the purchase flow (Shop), so the equip system is validated cheaply before Shop adds transaction complexity. **Implementation call (2026-07-15):** accessories are **emoji overlaid on the blob at a slot** (HEAD/FACE/NECK), not hand-drawn Canvas shapes — matches the mood-emoji overlays and keeps the catalog cheap; the tradeoff is emoji don't recolor with the theme like the blob does. The equipped accessory is hoisted at the composition root via a `CompositionLocal` so every `Mascot()` wears it without threading a param (same idea as the theme). Confirmed with Quinn before building.
...
- **Stage 7 — Anti-Cheat Hardening ✅ complete:** Adversarial pass on Stage 1's detection logic — flipped detection from fail-open to fail-closed, voided force-closed sessions (per-tick heartbeat + reconcile), blocked Stop during a group alarm, verified the 2-min mute cap + airplane-mode record. All 4 steps emulator-verified; plan archived at `docs/archive/STAGE7_PLAN.md`.
- **Stage 8 — Social Refinement (Groups & Friends) ✅ complete:** (Re-scoped from the old "Polish & Portfolio Packaging" at Quinn's request — packaging became Stage 9.) Discord-like group organization (tabbed Lobbies·Chat·Members + settings sheet, owner+admin roles), friend remove/profile-view/live-status, and an app-wide presence doc. All 7 steps built, rules deployed, emulator-verified; plan archived at `docs/archive/STAGE8_PLAN.md`.
- **Stage 9 — Polish & Portfolio Packaging ✅ complete (2026-07-16):** Visual/UX polish + stability for a recruiter audience. Punch-list of demo-impact fixes (session-summary payoff, roster naming, achievements loading skeleton, Home idle layout, chat timestamps, feed-verify) **all cleared**, plus a full **in-app dark mode** (Light/Dark/System toggle). No plan doc — tracked in `PROGRESS.md`. **Packaging tail done:** `README.md` (detailed, product-forward, demo-GIF hero + paired light/dark gallery), `docs/screenshots/`, and `docs/demo.gif`. This is the final staged milestone — the app is feature-complete.
